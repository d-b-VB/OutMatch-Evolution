...
try {
  const html = chromium(["--dump-dom", `${baseUrl}/`]);
  ...
  const acceptance = chromium(["--virtual-time-budget=30000", "--dump-dom", `${baseUrl}/browser-acceptance/acceptance.html`]);
  ...
  for (const viewport of [{ name: "desktop", size: "1440,1000" }, { name: "phone", size: "390,844" }]) {
    for (const screen of [{ name: "dashboard", hash: "#overview" }, { name: "active-run", hash: "#run-progress" },
      ...
      chromium([`--window-size=${viewport.size}`, `--screenshot=${path}`, `${baseUrl}/browser-acceptance/scenarios.html${screen.hash}`]);
