...
const coordinateIndex = new Int8Array(49).fill(-1);
for (let index = 0; index < BOARD_CELLS.length; index += 1) {
  const [q, r] = BOARD_CELLS[index];
  coordinateIndex[(q + BOARD_RADIUS) * 7 + r + BOARD_RADIUS] = index;
}
const key = ([q, r]) => q < -3 || q > 3 || r < -3 || r > 3
  ? -1 : coordinateIndex[(q + BOARD_RADIUS) * 7 + r + BOARD_RADIUS];
const neighbours = BOARD_CELLS.map(cell => DIRECTIONS
  .map(([dq, dr]) => [cell[0] + dq, cell[1] + dr])
  .filter(candidate => key(candidate) >= 0));
const distances = BOARD_CELLS.map(a => BOARD_CELLS.map(b => Math.max(
...
export function hexDistance(a, b) {
  const aIndex = key(a); const bIndex = key(b);
  if (aIndex >= 0 && bIndex >= 0) return distances[aIndex][bIndex];
  return Math.max(
    Math.abs(a[0] - b[0]),
    Math.abs(a[1] - b[1]),
    Math.abs((-a[0] - a[1]) - (-b[0] - b[1]))
...
export function deploymentSpots(state, side) {
  ...
  return neighbours[key(BASE[side])].filter(position => occupied[key(position)] === undefined);
}
...
function occupancy(state) {
  const result = new Array(BOARD_CELLS.length);
  for (let index = 0; index < state.units.length; index += 1) result[key(state.units[index].pos)] = state.units[index];
  return result;
}

function isInEnemyPikeZone(state, position, side, occupied = occupancy(state)) {
  return neighbours[key(position)].some(cell => {
    const unit = occupied[key(cell)];
...
export function legalMoves(state, unit) {
  ...
  if (unit.typ === "C") {
    ...
    while (queue.length > 0) {
      ...
      for (const candidate of neighbours[key(position)]) {
        const candidateKey = key(candidate);
        if (seen.has(candidateKey)) continue;
        seen.add(candidateKey);
        const occupant = occupied[candidateKey];
  ...
  if (unit.typ === "A") {
    return neighbours[key(unit.pos)].filter(cell => occupied[key(cell)] === undefined);
  }
  return neighbours[key(unit.pos)].filter(cell => {
    const occupant = occupied[key(cell)];
...
export function archerTargets(state, unit) {
  ...
  const occupied = occupancy(state);
  return neighbours[key(unit.pos)]
    .map(position => occupied[key(position)])
    .filter(target => target?.side === enemyOf(unit.side));
}

function cloneSearchState(state) {
  const cloneCounts = counts => ({ P: counts.P, A: counts.A, C: counts.C });
  const cloneMetrics = metrics => ({
    trained: cloneCounts(metrics.trained), pokeActions: metrics.pokeActions, pokeKills: metrics.pokeKills,
    killsByAttacker: cloneCounts(metrics.killsByAttacker), victimsByType: cloneCounts(metrics.victimsByType)
  });
  return {
    units: state.units.map(unit => ({ ...unit, pos: [unit.pos[0], unit.pos[1]] })),
    round: state.round, turn: state.turn, pending: { ...state.pending }, nextUnitId: state.nextUnitId,
    captured: { ...state.captured }, metrics: { R: cloneMetrics(state.metrics.R), B: cloneMetrics(state.metrics.B) },
...
export function legalActions(state, unitId) {
  ...
  if (unit.typ === "P") {
    const occupied = occupancy(state);
    for (const position of neighbours[key(unit.pos)]) {
      const victim = occupied[key(position)];
    ...
    const targets = archerTargets(state, unit);
    ...
    for (const destination of moves) {
      actions.push({ kind: "move", unitId, destination });
      for (const position of neighbours[key(destination)]) {
        const target = occupied[key(position)];
        if (target?.side === enemyOf(unit.side)) {
          actions.push({ kind: "moveshoot", unitId, destination, targetId: target.id });
        }
      }
    }
    for (const target of targets) {
      const targetKey = key(target.pos);
      for (const destination of neighbours[key(unit.pos)].filter(cell => {
        const cellKey = key(cell);
        return cellKey === targetKey || occupied[cellKey] === undefined;
      })) {
...
function isCaptureAction(state, side, action, occupied) {
  ...
  return occupied[key(action.destination)]?.side === enemyOf(side);
}
...
export function lookahead(state, side, genome, depth = 3, history = []) {
  let beam = [{ score: 0, state: cloneSearchState(state), actions: [] }];
  ...
  for (let ply = 0; ply < searchDepth; ply += 1) {
    ...
    for (const node of beam) {
      ...
      for (const action of actions) {
        ...
        const nextState = cloneSearchState(node.state);
  ...
  return beam[0] ?? { score: 0, state: cloneSearchState(state), actions: [] };
}
...
export function planTurn(state, side, genome, depth = 3) {
  const plannedState = cloneSearchState(state);
  const actions = [];
  let score = 0;
  while (plannedState.units.some(unit => unit.side === side && unit.active)) {
    ...
    const before = cloneSearchState(plannedState);
