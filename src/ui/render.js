      ...
      <main id="main-content">
        ...
        <section class="populations-panel" id="populations"><div><p class="eyebrow">Immutable lineage</p><h2>Populations</h2>
          ...
          ${generation?.checkpoint?.population?.length ? `<div class="population-browser"><div class="population-summaries">${populationView.summaries.map(summary => `<button class="population-summary ghost" type="button" data-population="${escapeHtml(summary.population)}"><b>${escapeHtml(summary.population)}</b><span>${summary.count}</span></button>`).join("")}</div>
            ...
            <div class="population-table" tabindex="0"><table><caption>Generals in the selected immutable generation</caption><thead><tr><th>Rank</th><th>General</th><th>Population</th><th>Origin</th><th>Fitness</th><th>Training</th><th>Kills</th><th>Pokes</th><th>Parentage</th></tr></thead><tbody>${populationView.rows.map(row => `<tr><td>${row.rank ?? "—"}</td><td><button class="general-detail-link" type="button" data-general-id="${escapeHtml(row.id)}">${escapeHtml(row.name)}</button><small>${escapeHtml(row.id)}</small></td><td>${escapeHtml(row.population)}</td><td>${escapeHtml(row.origin)}</td><td>${row.fitness ?? "—"}</td><td>${row.training ?? "—"}</td><td>${row.kills ?? "—"}</td><td>${row.pokes ?? "—"}</td><td>${escapeHtml(row.parentage)}</td></tr>`).join("")}</tbody></table></div>
            ${populationView.detail ? `<aside class="general-detail"><p class="eyebrow">General detail</p><h3>${escapeHtml(populationView.detail.row.name)}</h3><dl><dt>ID</dt><dd>${escapeHtml(populationView.detail.row.id)}</dd><dt>Origin</dt><dd>${escapeHtml(populationView.detail.row.origin)}</dd><dt>Parents</dt><dd>${escapeHtml(populationView.detail.row.parentage)}</dd><dt>Fitness</dt><dd>${populationView.detail.row.fitness ?? "—"}</dd></dl><details><summary>Fitness & unit behavior</summary><pre>${escapeHtml(JSON.stringify(populationView.detail.fitness, null, 2))}</pre></details><details><summary>Genome</summary><pre>${escapeHtml(JSON.stringify(populationView.detail.genome, null, 2))}</pre></details><details><summary>Provenance</summary><pre>${escapeHtml(JSON.stringify(populationView.detail.provenance, null, 2))}</pre></details><button id="intervene-from-general" class="ghost" type="button" data-general-id="${escapeHtml(populationView.detail.row.id)}">Queue intervention</button></aside>` : ""}</div>`
