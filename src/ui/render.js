    ...
    <div class="lab-shell">
      ...
      <main id="main-content">
        ...
        <section class="progress-panel" id="run-progress" aria-live="polite" aria-busy="${runControls.active}"><div><p class="eyebrow">Durable execution</p><h2>${progressSummary ? escapeHtml(progressSummary.phaseLabel) : "No generation in progress"}</h2>
          <p class="intro">${progressSummary ? `Working toward ${escapeHtml(progressSummary.targetGeneration ?? "the next generation")}. Every displayed result is already represented by a safe local checkpoint.` : "A started generation will report its last safely committed tournament boundary here."}</p>
          ${runControls.active ? '<div class="battle-orbit" role="img" aria-label="Tournament simulation active"><span class="battle-core" aria-hidden="true">⚔️</span><span class="battle-unit horse" aria-hidden="true">🐎</span><span class="battle-unit archer" aria-hidden="true">🏹</span><span class="battle-unit shield" aria-hidden="true">🛡️</span></div>' : ""}</div>
          ${progressSummary ? `<div class="progress-card"><div class="progress-heading"><span id="progress-count">${progressSummary.currentTotal ? `${progressSummary.currentCompleted} / ${progressSummary.currentTotal} current-stage games` : "Between deterministic stages"}</span><b id="progress-percent">${progressSummary.percent}%</b></div>
            ...
            <div id="worker-activity" class="operation-status" role="status">${liveProgress ? `<b>Worker activity</b><br>Fight ${liveProgress.completed} of ${liveProgress.total} observed<br><span id="underway-fights">${escapeHtml(`${(liveProgress.underway ?? []).length} ${(liveProgress.underway ?? []).length === 1 ? "game" : "games"} underway — ${(liveProgress.underway ?? []).map(game => `${game.redId} vs ${game.blueId}`).join(" · ") || "awaiting the next dispatch"}`)}</span><br><small>Schedule ${liveProgress.scheduleIndex} · ${escapeHtml(formatProgressTimestamp(liveProgress.observedAt))}</small><div id="first-fight-log">${(liveProgress.firstFightActivity ?? []).slice(-8).map(move => `<span>${escapeHtml(move)}</span>`).join("")}</div>` : '<span id="underway-fights"></span><div id="first-fight-log"></div>'}</div></div>`
